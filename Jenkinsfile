	...
	stage('Build Version'){
        ...
        stage('Docker build & push image to AWS ECR') {
		    ...
		    withCredentials([string(credentialsId: 'ARTIFACTORY_USER', variable: 'ARTIFACTORY_USER'),
                               string(credentialsId: 'ARTIFACTORY_TOKEN', variable: 'ARTIFACTORY_TOKEN')]) {
                script {
                docker.build("wipp_backend", "--build-arg SOURCE_FOLDER=. --build-arg BUILD_VERSION=${BUILD_VERSION} --build-arg ARTIFACTORY_USER=${ARTIFACTORY_USER} --build-arg ARTIFACTORY_TOKEN=${ARTIFACTORY_TOKEN} --no-cache ./")  
